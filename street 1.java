...
public class CNM extends JPanel implements Runnable {
...
int LONR1 = 250;
int LONR2 = 320;
int tvel = 1;  // to modify velocity upon receiving green or red light
public static int flag; // to check if SL msg has been received 
int FinalPort = 5000;// should be taken from db
int greents;
static int already;
private int olength,owidth,oarea;
private int density;

public CNM(int xvelocity, int yvelocity) {
   color = Color.black;
    diameter = 5;
    delay = 40;
    x = 550;
...
public void move() {
	if (y >= LOA) // loa
 		{
			y = y + tvel; // keeping the velocity constant
			if ( check == 0)
			{
...
public class TestBroadcast extends Thread {
  //  String message="1:123";
	...
	public void initialise()
	{
	 ...
	 public void run() {
	        try {
	        	...
	        	addr1= new  InetSocketAddress("172.16.31.255", 9875);
	...
	class E extends Thread {
	    ...
	    public void run() {
	        try {
	        	...
	        	while(y < (LOD - 30))   // some point before lod so that it wont get stuck in receive line
	        	{
	        	//System.out.println("Receiving Block");
	        	byte[] rdata = new byte[20];
	    		DatagramSocket newsocket = new DatagramSocket(9875);
	...
	void receiveEGN(){
		try {
			...
			if (minstreet == StreetId)
			{
				...
				System.out.println("maxstreet:" +maxstreet);
	...
	{ 
		...
		calcDensity();
		System.out.println("here");
		System.out.println(IdIndex[0][1]);
		slv = this.redstamp * (this.egn + 10/ (this.egn + this.engn));
		return slv;
	}
